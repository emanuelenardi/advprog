...
% \begin{frame}[t]{Le conversioni di tipo [1/2]}
...
% \alert<4>{Due aspetti importanti di una conversione di tipo sono se avviene \textbf{implicitamente} (automaticamente) o \textbf{esplicitamente}, e se \textbf{la rappresentazione dei dati sottostante viene convertita da una rappresentazione in un'altra}, o \textbf{se una data rappresentazione è semplicemente reinterpretata} come la rappresentazione di un altro tipo di dati.}
% % In generale, possono essere convertiti sia i tipi di dati primitivi che composti.

% \end{frame}

% \begin{frame}[t]{Le conversioni di tipo [2/2]}

% Ogni linguaggio di programmazione ha le sue regole su come i tipi possono essere convertiti.
% I linguaggi con tipizzazione forte (\foreign{strong typing}) in genere eseguono poche conversioni implicite e scoraggiano la reinterpretazione delle rappresentazioni, mentre i linguaggi con tipizzazione debole (\foreign{weak typing}) eseguono molte conversioni implicite tra i tipi di dati.
% Un linguaggio con tipizzazione debole spesso consente di forzare il compilatore a interpretare arbitrariamente un elemento con rappresentazioni diverse, questo può essere un errore di programmazione non ovvio o un metodo tecnico per gestire direttamente l'hardware sottostante.
% \pause

% Nella maggior parte delle lingue, la parola \enquote{coercizione} (\textbf{\foreign{coercion}}) \textbf{viene utilizzata per denotare una conversione implicita}, sia durante la compilazione che durante il \foreign{runtime}.
% \pause
% Ad esempio, in un'espressione che mescola numeri interi e in virgola mobile (come \texttt{5 + 0.1}), il compilatore convertirà automaticamente la rappresentazione intera in rappresentazione in virgola mobile in modo che le frazioni non vengano perse.
% \pause
% \textbf{Le conversioni di tipo esplicite sono indicate scrivendo codice aggiuntivo} (ad esempio aggiungendo identificatori di tipo o chiamando funzioni integrate) o scrivendo funzioni di conversione per il compilatore da utilizzare quando, in assenza delle stesse, il compilatore interromperebbe l'esecuzione con una mancata corrispondenza del tipo (\foreign{type mismatch}).
...
Nella famiglia dei linguaggi C, la parola \enquote{\foreign{cast}} si riferisce tipicamente ad una conversione di tipo esplicita (al contrario di una conversione implicita), causando qualche ambiguità sul fatto che si tratti di una reinterpretazione di un \foreign{bit-pattern} o di una conversione di rappresentazione di dati reali.
